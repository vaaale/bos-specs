...
- **FR-014**: *(Extracted to `031-setup-wizard` on 2026-07-28 — full detail of the first-run configuration wizard, its 6 steps, git-repo setup, and marketplace pre-population now lives there. Retained here only as a pointer so the FR numbering stays stable: on first startup a configuration wizard MUST appear before the desktop is accessible, and MUST NOT appear again once setup is marked complete.)*
...
- **FR-017**: *(Extracted to `030-settings-mcp-servers` on 2026-07-27 — full detail of MCP server configuration, transports, and agent-tool parity now lives there. Retained here only as a pointer so the FR numbering stays stable for other specs' cross-references: configured servers remain auto-exposed to the agent per `011`, and to the gateway per `014`.)*
- **FR-018**: All server boot-time initialization MUST live in exactly one file — `src/instrumentation.ts` — whose whole body is gated on `process.env.NEXT_RUNTIME === "nodejs"`. There MUST NOT be a root-level `instrumentation.ts`, nor a sibling boot module (e.g. `instrumentation.node.ts`) that Next.js is assumed to discover.
  - Next.js calls the `register()` of exactly one file, resolved from **either** `<root>/instrumentation.ts` **or** `<root>/src/instrumentation.ts`; when both exist one silently wins, and which one is not stable across machines. It never loads sibling files by naming convention.
  - This has silently disabled BOS's entire boot sequence — installed-service autostart, plugin loading, and the scheduler daemon — **twice**: once by renaming the hook to `instrumentation.node.ts`, and once by adding a root-level hook that displaced `src/instrumentation.ts`. Both times there was no error and no warning; the code looked correctly wired.
  - After adding or changing boot-time logic it MUST be verified to have actually run — e.g. the expected components appearing in `data/logs/timeline-*.jsonl` — not merely to compile.
...
- *(SC-007 — MCP server configuration testing — moved to `030-settings-mcp-servers` SC-001. Numbering retained for stability.)*

## Notes

- Detailed sub-systems: `001-build-studio`, `002-memory`, `003-self-improvement`, `004-browser-automation`, `005-self-modification`, `006-data-isolation`, `007-gitfs`, `008-self-testing`, `009-installed-apps`, `010-documentation`, `030-settings-mcp-servers` (MCP server configuration, extracted from FR-017 on 2026-07-27). Principles: the constitution.
