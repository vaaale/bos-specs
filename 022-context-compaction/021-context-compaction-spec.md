...
- **FR-014**: If the 020 fast-loop module is present, the summarization job MUST first invoke the fast-loop review for this conversation (idle threshold waived, same code path as 020 FR-009) covering turns up to the boundary, and only then summarize. If absent or failing, proceed without it (log the skip) — compaction MUST NOT hard-depend on 020.
