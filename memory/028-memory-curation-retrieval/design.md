...
| VII. Respect Boundaries | ✅ **No new external dependency** — embeddings reuse the already-present `openai` SDK (its `.embeddings` resource), consistent with `llm.ts`'s `openaiClient()`. `provider.json` already stores the LLM key; the embedding key follows the same storage pattern (no `package.json`/lockfile/build change). |
...
- `topics.ts`: 
  ...
  - `addTopicEntry` / `replaceTopicEntry`: the hard `Over budget` reject becomes **accept + set `consolidate` flag** (T2). `currentBudget` is redefined to measure entry content only (excluding the marker/digest chrome) so the flag never inflates the number.
...
- `consolidate.ts`: the slow loop gains read + lifecycle capability — `topic_read` (read a topic's full entry list, FR-005), `topic_set_digest`, and `topic_supersede`. The preamble lists **flagged** topics (so the pass knows what to reorganize). The pass merges/dedups/splits via the **existing atomic single-entry ops** (never a whole-file rewrite — see ADR-2) and **clears the flag only on success** (FR-007). Contradiction detection is added to the system prompt as a background consolidation behavior (T4), not an inline check.
...
**Modify**
- `src/lib/agent/memory/topics.ts` — soft budget (accept+flag), `Topic.consolidate` flag (parse/serialize, non-`>` marker), `TopicEntry.state`/`supersededBy` lifecycle, `setTopicDigest`, `supersedeTopicEntry`, redefined `currentBudget`.
...
- `src/app/api/agent/provider/test/route.ts` — probe embeddings availability (1-token embed) alongside the existing LLM completion probe.
- `src/lib/config/registry.ts` — `ai-provider` `fields[]`: add embedding fields (`secret: true` on the key).
- `src/lib/agent/capabilities-registry.ts` — add `memory_replace` / `memory_remove` capability entries (parity with `memory_save`).
...
```
- **Key = the entry id**, which in this subsystem is `hashId(text)` (FNV-1a of the entry text, already computed by `topics.ts` and present on every parsed `TopicEntry`). Because the id *is* a content hash, **recompute-on-text-change (FR-013) falls out by construction**: unchanged text → same id → cache hit; changed text → new id → miss → compute, and the old id is simply no longer referenced.
...
2. **Reorganize as atomic single-entry ops**: merge near-dups = `topic_remove_entry` ×2 + `topic_add_entry` ×1 (the merged text); drop stale = `topic_remove_entry`; **split** = `topic_create` (sibling) + `topic_add_entry` (each moved entry) + `topic_remove_entry` (from source); **refresh digest** = `topic_set_digest`. Every one of these is a single atomic `vfs.writeText` (temp+rename) that also rebuilds the index — so **no topic is ever left in a partial/corrupt state** (ADR-2).
3. **Clear flag only on success**: the pass clears the source topic's `consolidate` flag in a final write *after* the reorganization completes (FR-007). If the pass is interrupted mid-sequence, each file is still valid (old or new content), and the flag **survives** (it's still in the over-budget source), so the next hourly pass idempotently completes the job. The existing per-agent overlap lock + 30-min staleness prevents concurrent passes racing the same agent.
...
**Query embedding**: one `embed()` call per search (the corpus-side vectors are cached). If that call returns `null`, the whole search degrades to sparse+recency+importance for that query — still no error.
