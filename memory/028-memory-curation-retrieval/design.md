...
- **Capability context (N3)**: register `memory_replace`/`memory_remove` with `context: "action"` in `capabilities-registry.ts`, matching the existing `memory_save`/`memory_recall`/`memory_search` entries. These are live server tools (used by `server/memory.ts` and `tool.ts`), so `"both"` would be *defensible*; we pick **`"action"` for parity** with the whole `memory_*` family rather than introducing a mixed context within one group.
...
### ADR-2 — Holistic consolidation via *invariant-enforcing* atomic single-entry ops (no whole-file rewrite; no false idempotency claim)
...
- **Options**: (a) add a `topic_reorganize(topic, newEntries[])` whole-file-replace primitive (simpler model, but a multi-entry atomic write + it violates the anti-collapse rule and creates a new corruption surface); (b) give the loop `topic_read` + express merge/split as *sequences* of the existing atomic single-entry ops, and define the pass as **invariant-enforcing** rather than "idempotent."
- **Decision**: (b).
- **Consequences**: The anti-collapse rule is preserved (no whole-file primitive is added); each op is one atomic temp+rename write. The pass is defined to preserve three invariants across **any** intermediate state: **no entry lost, no entry duplicated, every entry's id/content-hash conserved** — a split *moves* entries (never deletes-then-recreates); a merge folds N→1 **only** via the existing atomic remove×N + add×1, and the merged entry is a **new** entry so nothing is lost (the *meaning* is preserved by the LLM's judgment, which is acceptable). An intermediate-but-**CONSISTENT** state (source shrunk, sibling partially populated, digest stale) is **permitted and is NOT corruption**. **We do not claim the interrupted job is idempotently re-run**: an LLM re-run sees an already-modified starting state it cannot reconstruct into the original intended set. The guarantee is instead **no entry lost/duplicated + the surviving flag ⇒ monotonic convergence to a consistent state** (a finished split is never undone; the flag drives the next pass). This means **SC-006's literal "fully unchanged or fully reorganized (no partial state)" is not literally met** — an intermediate-but-consistent state is a valid, permitted outcome — so that wording needs refinement (proposed in the "Spec-level finding (needs user decision)" section below; not baked into the spec here).
...
- **Decision**: (b).
- **Consequences**: No new *declared* dependency. Reuses existing base-URL normalization and auth handling. The `openai` SDK works against any OpenAI-compatible endpoint (local, OpenAI, Codex, Responses-compatible) — exactly the provider set in `provider-meta.ts`. Anthropic has no such resource → `embed()` returns `null` → degradation. (Hand-rolled fetch was rejected: it would duplicate the normalization/auth logic `llm.ts` already handles and add a maintenance surface.) **Dependency caveat (S6)**: `openai` is imported by `llm.ts` but is **not declared in `package.json`** (it resolves today as a hoisted transitive); `embed()` inherits this. It is not a *new* dependency, but the honest position is to declare `openai` explicitly in `package.json` (one line) — which **would touch the lockfile**, a small, explicitly-flagged Constitution VII tradeoff rather than an unconditional "no lockfile change."
...
- **R1 — Tool-name collision with a stale duplicate (fix the collision here; retire the duplicate separately).** The real issue is a **tool-name collision**, not a second curated store. There is **no** `target:"user"|"memory"` curated store — `/api/memory`'s POST only handles `action:"setPreferences"` and `target:"topic"` (the **topic** store). The legacy CopilotKit `MemoryActions.tsx` (`src/components/agent/MemoryActions.tsx`) is a **stale, half-broken duplicate** of the same topic surface: its `memory_save` always errors (it sends `target:"user"|"memory"`, which the route rejects on fall-through), and its no-arg `memory_recall` reads `res.user`/`res.memory` (which the route never returns). Both `MemoryActions.tsx` and `server/memory.ts` register `memory_save`/`memory_recall`/`memory_search`. **Conclusion**: `MemoryActions.tsx` is a stale duplicate to **retire in a separate cleanup (out of scope here)**; this feature scopes to the **live topic surface** (`server/memory.ts` + `tool.ts`) and **must ensure the new `memory_replace`/`memory_remove` tool names do not collide with the legacy set** (they don't today). **Scope decision**: leave `MemoryActions.tsx` untouched in this feature.
- **R2 — Soft budget + slow loop latency.** Between a soft-budget overflow and the next hourly slow-loop pass, a topic can sit over budget (unbounded in the worst case if the loop is disabled or has no credentials). This is the accepted trade for unblocking the write path (SC-001). Mitigation: re-setting the flag is a no-op (it just stays set) and the pass runs hourly (and, per M1, the flagged-topics path fires even with zero pending episodes); the slow loop already no-ops without `hasCredentials()`. Open: should overflow also *nudge* the agent in-band to call `memory_replace`/`memory_remove` (a soft in-band signal, not a hard reject)? Currently designed as silent flag-only.
- **R3 — Embedding cost/latency on first search (bounded by default, N2).** Resolved: the first-search dense burst is **bounded by default** — embed only the **top-K sparse (BM25) candidates** to seed the dense signal (see §5.5), then cache thereafter. This caps cold-cache cost at one query embed + ≤K corpus embeds per cold search. K is a config-exposable constant (~8–10).
...
- **R8 — Secret surface growth; two DIFFERENT redaction mechanisms, both required (S5).** Adding `embeddings.apiKey` to `provider.json` extends the existing key store. Two read paths redact it via **different** mechanisms, and **both must be set**: (1) the **provider API route** reads `getProviderConfigView()` → `hasEmbeddingKey` (**boolean only**); (2) the **config registry** (`config/registry.ts` `ai-provider` load) calls `getProviderConfig()` (**full, with secrets**) and relies on **`secret: true`** on the embedding-key field declaration to redact. A security reviewer should sign off that *both* the view's boolean and the registry's `secret: true` are in place — setting only one leaves the other path exposing the key.
...
## Spec-level finding (needs user decision)

The design above (per the task's instruction) does **not** edit `spec.md`. However, an honest resolution of the M2 must-fix (interruption semantics, ADR-2) shows that the current wording of **SC-006** is not literally satisfiable, and needs a one-line refinement.

- **Current SC-006**: "Consolidation of a topic does not corrupt it on interruption: after a forced mid-pass failure, the topic is either fully unchanged or fully reorganized (no partial state), and the flag survives for re-run."
- **Why it is not literally met**: reorganization is expressed as *sequences of atomic single-entry ops* (the ACE anti-collapse rule forbids a whole-file write). A mid-pass failure therefore leaves an **intermediate-but-consistent** state (e.g. source topic shrunk, a split sibling partially populated, digest not yet refreshed). That state is valid and lossless, but it is neither "fully unchanged" nor "fully reorganized." No whole-file primitive can be added to make the two end-states the only outcomes without violating the anti-collapse rule the design correctly preserves.
- **Proposed revised SC-006** (presented for the user's approval — **not** baked in):
