...
- **FR-004**: The fast loop MUST run as a **System-category JobDefinition** in the **Unified Scheduler Engine** (see `bos-system-specs/scheduler/spec.md`), persisted like every other job in the single unified store `/Documents/System/scheduler-jobs.json`. Default interval every 2 min; handler `{ kind: 'internal', ref: 'memory.fast-loop' }`; no client-side hook and no agent volition. On startup the memory subsystem MUST ensure this job exists in the unified store (idempotent seed by stable id `system:memory.fast-loop`); it MUST NOT maintain a separate persistence file for scheduling.
...
- **FR-010**: The slow loop MUST run as a **System-category JobDefinition** in the **Unified Scheduler Engine**, persisted in the single unified store `/Documents/System/scheduler-jobs.json` (stable id `system:memory.slow-loop`, handler `{ kind: 'internal', ref: 'memory.slow-loop' }`), default hourly, and MUST exit immediately (no LLM call) when no `pending` episodes exist. Like the fast loop, the memory subsystem seeds this JobDefinition idempotently on startup — it does NOT keep its own scheduler persistence.
...
| Integration-config migration | one-time boot step invoked by the engine (Phase 0) | Migrates legacy per-integration polling configs into the unified store (see scheduler spec FR-016). Not memory-loops-specific but blocks memory-loops rollout. |
...
| Scheduler job seeding | boot-time seed of two System-category JobDefinitions into `/Documents/System/scheduler-jobs.json` | Stable ids `system:memory.fast-loop`, `system:memory.slow-loop`; idempotent; do NOT create a separate persistence file for these. Handler kind `internal` with the refs above. Lock file for slow-loop overlap remains under `/Documents/Memory/.consolidate.lock`. |
