...
- `reentrancy.ts` — the self-heal-origin tag + predicate (ADR-4).
...
- `src/plugins/self-heal/init.ts` + the plugin def (like `bos-compaction`): `afterToolCall` (feeds the repeated-failure tracker + emits `self_heal.trigger` for non-allowlisted hard errors) and `onError`. Registered via `instrumentation.ts` + `api/plugins`. Carries the re-entrancy tag so Diagnostician runs are ignored (ADR-4). The 300s/3-count rolling window lives here (per-conversation, in-memory) and calls `selfHealIntake`.
...
**Why not a 002 workflow here (repeated for the ADR):** a 002 run is (a) LLM-node-driven, (b) fire-and-poll with no in-run suspension/resume primitive, (c) unable to seed a *conversation* and set its `activeFeatureBranch`, and (d) unable to hold a cross-run in-process mutual-exclusion slot. The deterministic spine + case store + `runSubAgent` on a persistent conversation gives *all four* using mechanisms BOS already has.
...
- **Ownership predicate.** `listInstalledItems()` / `data/user-apps/items/<id>/` presence (`src/lib/apps/store.ts`, `src/lib/gitfs/store.ts`). Class `d` vs `d-bis` is decided by whether the item exists there (the Diagnostician's `app_list` tool + the spine's server-side check).
...
**Context.** The spec's *Assumptions* name "the Workflow Manager service (with event-triggered runs, per spec 002)" as the fast-spine vehicle, and the entities call the spine "a workflow." But (a) `docs/dev/architecture-overview.md` §14 retired the core workflow engine; (b) the 002 service (`specs/user-specs/workflow-manager/001-...`) is an **LLM-agent-node** engine with **no deterministic-code node** and **no event-triggered-run capability** (verified: nothing subscribes to events to start a run); (c) the spine must be deterministic (only the Diagnostician step is LLM), must hold a **cross-run in-process mutual-exclusion slot**, and must **set a conversation's `activeFeatureBranch`** + seed conversations — `src/lib` server capabilities a worker-thread service cannot perform. A single synchronous "workflow run" also cannot host a multi-minute Diagnostician LLM step, because a 034 core executor has a `timeoutMs` and the dispatch engine settles on it.

**Options.**
- *A — 002 workflow service with a deterministic "tool" node driving the spine.* Rejected: 002 has no deterministic node type, no event trigger, no cross-run slot, and cannot seed/conversation/branch. It would require extending 002 substantially — the opposite of "reuse what exists."
...
### ADR-4 — Re-entrancy guard: tag self-heal runs, filter at the capture points
...
**Decision.** Propagate a **self-heal-origin marker** through the run context and filter on it at every capture point:
- The Diagnostician run is launched with `runSubAgent(…, { conversationId: <diagnosticianRunConv>, … })` where the conversation/context carries `selfHeal: { role: "diagnostician", caseId }`. The `bos-self-heal` hook plugin's `afterToolCall`/`onError` checks the run's origin: if it's a self-heal diagnostician run, **skip** (do not emit `self_heal.trigger`).
- The slow-path BS conversation carries `selfHeal: { role: "pipeline", caseId }`. Tool errors *inside* the pipeline are the Developer's job (they surface as build/test failures → case `failed`), not new triggers — the hook also skips pipeline-origin runs. (Optionally, a *distinct* "pipeline tool error" log is emitted for observability but never creates a case.)
- Workflow-timeout and log-error triggers are already scoped to *user/workflow* namespaces, not the self-heal run; the guard there is that the 002 service and the logging sink do not tag their events as self-heal-origin (they can't be — they're outside the self-heal run). The belt-and-braces rule: **any event whose source/payload carries `selfHeal.role` is never intake'd.**

**Consequences.** + Uniform single predicate at one choke point (`intake.ts` drops self-heal-origin, and the hook drops before emitting). + Cheap: a context field, no new subsystem. − The hook plugin must be able to read the run's origin — confirm the `afterToolCall` hook signature exposes the run context (if not, thread it via the same `runId`-keyed registry the delegation system uses). This is an implement-step verification, flagged in §7. − If the marker is dropped, recursion is *bounded* by the dedupe window + cost cap, not infinite — but the guard makes it not even reach dedupe.

---

### ADR-5 — Cost cap = per-case ledger written at run completion, checked at enqueue (not a live meter)

**Context.** FR-020: total LLM tokens across all self-heal cases in a calendar day ≤ `costCapPerDay`; when reached, **new** triggers are **queued** (not dropped) and processed next day; queue bounded (≤100, 7-day TTL); reset midnight UTC; emit `cost_cap_evicted` on eviction. **Verified: BOS has no central token meter** (`total_tokens`/`usage` surfaced to a central store does not exist; the LLM paths — `runSubAgent` local runs and the `claude-runner` stream-json `result` line — carry usage but nothing aggregates it).

**Options.**
- *A — Real-time metering across all LLM calls.* Rejected: would require instrumenting every provider path in BOS for a feature that only cares about *self-heal-origin* runs; disproportionate blast radius.
- *B — A self-heal-scoped ledger: each self-heal run (Diagnostician run, BS pipeline run, Developer run) records its usage into the case store's cost ledger when it completes (the usage is available in the `runSubAgent` result / claude `result` line); the cap is enforced at *intake/enqueue* against the sum of today's ledger entries. **Chosen.**
- *C — Estimate from context window.* Too coarse to enforce a token cap honestly.

**Decision.** Option B. `cost.ts` maintains `ledger: CostLedgerEntry[]` in `data/self-heal/index.json` (`{caseId, role, tokens, at}`), pruned to the current UTC day at read time. `capExhaustedForToday()` sums the day's entries vs `costCapPerDay`. At intake, if exhausted → the case goes to the **bounded slow-path queue** (or a *pending-retry* queue for cheap classes), **not dropped**; at the UTC midnight rollover the next intake processes them FIFO. **A single in-flight case may overshoot the cap** — this matches FR-020's explicit "reached mid-pipeline: the current case completes; NEW triggers are queued." When a self-heal run's usage is unavailable (provider omits it), fall back to a conservative estimate from the final message/context length and log the estimate flag.

**Consequences.** + Cheap, self-heal-scoped, no global instrumentation. + Matches the spec's own mid-pipeline semantics exactly. − The cap is *enforced between cases*, not *within* one — an autonomous multi-hour pipeline is one ledger entry recorded at its end, so a very large single case can blow past the daily cap. That is acceptable per FR-020 (it's the "current case completes" clause) but worth stating to the user. − Usage fidelity depends on the provider returning it; the estimate fallback keeps the ledger honest but approximate. − The bounded-queue + TTL + `cost_cap_evicted` events live in `queue.ts` and are straightforward state on the ledger.
...
1. **R1 — Spec's "workflow" assumption is wrong; reconcile before `plan`.** §1/ADR-1. The *Assumptions* line "Workflow Manager service (with event-triggered runs, per 002)" does not describe 002's actual capability. Build Studio must update the spec's Assumptions (and the "workflow" wording in Entities/Stories) to "a deterministic bos-core coordinator" so spec and design agree. *This is the single most important reconciliation item.*
2. **R2 — Case id vs branch name (ADR-3).** The mockup's `EHS-<n>` ids are not valid `bos/*` branch segments. Decide the id scheme: keep a human `EHS-0141` id and derive the branch as `bos/self-heal-0141` (lowercased, digits). Confirm with the user; the branch slug must be stored on the case (`activeFeatureBranch`) and the human id separate.
3. **R3 — Hook plugin must see run origin (ADR-4).** Verify the `afterToolCall`/`onError` hook signature exposes the run's context/`runId` so the self-heal-origin marker is reachable. If the hook API does not expose it, thread the marker via the `runId`-keyed in-run registry the delegation system already uses (`src/lib/agent/subagents/in-run-agents.ts`). Implement-step verification.
...
5. **R5 — Cost fidelity (ADR-5).** Usage depends on the provider returning it; the estimate fallback is approximate. Decide the default `costCapPerDay` (mockup shows "50k" — is that 50,000 tokens? that's very small for a Diagnostician + full BS pipeline; likely 50k is a placeholder or means 50k *per cheap class* — confirm the unit and a realistic default). Flag to user.
...
| **2 · Case Detail — Scope-class-dependent Action Area** | `CaseDetail` + `ConsentCard`/`SuspendedCard`/`PreviewStatus` | Four action cards keyed by scope class: `a` (no durable change / Close), `b` (proposed skill/workflow diff / Approve edit / Dismiss — consent-gated, FR-010/011), `e` (preview status + branch + "Pin & open preview"; promote stays in Topbar per FR-024), and **suspended** (amber border + pulse — the binding visual for the HITL state; the pending question + answer input → `POST /api/self-heal?op=answer`). The `d` and `d-bis` cards follow the same pattern (d = notify-only notice; d-bis = `app_build` status + app id). |
