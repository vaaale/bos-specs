...
- The workflow engine is **fully re-implemented inside the Workflow Manager service** (user-approved): BOS's `src/lib/workflows/*` and `/api/workflows/*` routes are **retired**, and the worker owns the complete engine — node/step types (ephemeral, research, delegate, tool, ag-ui), dynamic routing, parallel scheduling (up to `maxConcurrentSteps`), persistence, and execution. This reverses the earlier "engine stays in BOS source" assumption (ADR-1). The feature is therefore **bos-core + marketplace-item**: a bos-core retirement of the old engine, plus a marketplace-item re-implementation as a service.
