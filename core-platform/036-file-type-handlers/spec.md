...
- **FR-002**: The OS MUST provide a **file-type-handler registry** that derives, per request, the set of file-handler declarations from **all currently-installed apps** (built-in and marketplace). Uninstalled apps' declarations MUST NOT appear in the registry (this holds by construction if the registry is derived from the live installed-app set on each query, rather than cached at boot).
...
- **FR-008**: In the Files app, right-clicking a file MUST show an **"Open with \<label\>"** entry for **each** installed app registered as a handler for the file's MIME type (render- or edit-capable, including hidden apps). Each entry is labelled "Open with \<label\>" where `<label>` is the handler's declared display label (falling back to the app's manifest `name` if none is declared), and shows the **target app's own manifest icon**. The entry for the currently selected handler is marked (e.g. a checkmark), per FR-010. A hairline divider separates the "Open with" group from the pre-existing menu actions.
...
- **A-8**: Handler declarations live in the app **manifest** (the self-describing `src/apps/<id>/manifest.ts` for built-ins and the item's `app.json` for installed apps), mirroring `AppManifest.eventHandlers` (spec 034). The registry is a **per-request derivation** from the live installed-app set (see design.md ADR-1) — no long-lived cache, no re-collection hooks; "reflect only installed apps" holds by construction. Persisting a user's per-type *selection* (FR-010) is the only durable state this feature introduces.
