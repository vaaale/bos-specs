...
- **Window delivery split** — `src/components/desktop/Window.tsx:287`: `manifest.kind ===
  ...
  invents. For iframes the line builds `IframeApp`'s `params` as
  `{ ...win.params, url: manifest.url, capabilities, origin }` — i.e. the iframe's `src` is
  the app's **entry page** (`manifest.url`), and any `win.params.url` is **overwritten** by it.
  That is why an installed handler's file is delivered as `path`/`action` (via `withFileParams`),
  never as an OS-supplied `url` (ADR-3 / §6).
...
- **MIME resolution** — `src/os/file-handlers.ts` `mimeForPath`/`fileBaseMime` (the
  authoritative **matching** map, ADR-8) — FR-004. `mimeForPath` returns
  `text/html; charset=utf-8`; `baseMime` normalizes it (ADR-4). The raw route's own larger
  **serving** map (`src/app/api/fs/raw/route.ts`) is separate and **not** used for matching —
  it only sets `Content-Type` on the bytes it streams. Consequence (ADR-8): an extension only
  in that serving map (e.g. `.pdf`, `.mp4`, `.mp3`) resolves to `application/octet-stream`
  under the matching map, so a handler declaring `application/pdf` would never match in the
  Files app. Do not reach for the serving map for matching.
- **Raw-file URL** — `src/lib/os-client.ts` `fsClient.rawUrl(path)` → `/api/fs/raw?path=`
  ...
  fetches the file's bytes. `html-viewer` already loads this URL in an `<iframe src>`, and the
  Files app's own in-app image `<img src>` uses it. The **launch contract's** `url` param is
  built client-side by `rawUrlFor` (`src/os/file-handlers.ts`) — the string-identical,
  no-`conversationId` form — because the param assembler is the client Files app (M-1/ADR-7); it
  does not import `os-client.ts` for this. Both produce the same URL for a bare `path`.
...
- `paramShape.url = "raw"` → **built-in (component) handlers only**: the OS sets
  `url = rawUrlFor(path)` (the file's bytes) as a **direct** launch param. This is html-viewer's
  case — it wants to render the document, so it gets the raw-file URL in `params.url`.
- `paramShape` absent / `url` omitted → OS sets only `path` + `action`. For an **installed
  (iframe) handler** these ride as `bos*` query params via `withFileParams` (the iframe's `src`
  stays the app's entry page — the OS does **not** supply a `url`; `Window.tsx` overwrites any
  `win.params.url` with `manifest.url`); for a built-in handler they are direct params. The
  installed handler fetches the bytes itself via the BOS SDK (A-4). This is the
  marketplace-editor case (follow-on, O-2).
- `paramShape.title = "basename"` → OS sets `title = basename(path)` (built-in: direct param;
  iframe: a `bos*` query param).

The **`"app"` token is dropped** (ADR-3): for an installed iframe the app URL is unconditionally
`manifest.url`, so a `paramShape` token that produced it was redundant — and, because `Window.tsx`
sets the iframe's `src` from `manifest.url`, an OS-supplied `url` for the iframe case would never
