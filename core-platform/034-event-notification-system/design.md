...
| **V. VFS Is Not the Source** | Event store persists as **files under `data/events/`** (runtime state excluded from the BOS source tree by `.gitignore`'s `/data` rule — verified, it is a live rule), NOT in the VFS and NOT in `src/`. Compliant — this is the "all runtime state persists as files under `./data`" convention. |
...
**Why there is no "service with its own network port" in this design (and why the `services.md` §11 reachability machinery is N/A here):** the event kernel is an in-process daemon — it has no listening socket, so the "a service's port is never reachable behind a reverse proxy / must use `wsPath`/`httpPath`" concern does not arise for the kernel itself. The *emitters* are worker-thread services that already exist with their own reachability; they reach the kernel by **loopback HTTP to BOS's own API** — the same plain loopback-HTTP-to-localhost pattern documented for a worker-thread service reaching the VFS (`docs/dev/os-shell/virtual-file-system.md`, "Why this matters beyond the Files app": a plain loopback HTTP call to BOS's `/api/fs` on `localhost`, no `@/` import) and for headless auth (`docs/dev/features/headless-client-auth.md`, "Worker-thread services": a loopback `POST /api/secrets/<service>/verify`, "the same shape as its existing `/api/fs` calls") — not by being exposed publicly. Nothing in this design shows a user a connection URL to a new port, so §11's three-scenario table does not apply. I state this explicitly because it is the one place a reviewer's instinct ("check §11") would mis-fire.
...
| `store.ts` | Persistence. Immutable event bodies in **per-month JSONL shards** (append-only); mutable per-event state (`processing`, `read`, `history`, per-handler status) in **per-month state files**; a projection **index held warm in memory** (UNBOUNDED — one entry per event for all time) flushed to `index.json` on **checkpoint/interval + graceful shutdown, never per-emit** (ADR-4); a **per-process mutex** serializing the (now bounded) hot-path writes. All durable writes via `writeFileAtomic`. | `src/lib/integrations/notifications/store.ts` (mutex + `writeFileAtomic`), `src/os/atomic-write.ts` |
...
| `src/os/types.ts` | Add optional `eventHandlers?: UIHandlerDeclaration[]` **and** `eventNamespaces?: string[]` (the FR-023 granted-namespace list) to `AppManifest` (UI handlers + namespace grants declared statically in a built-in app's `manifest.ts`; the same fields on installed apps' `app.json`; and `eventNamespaces` on a service's `service.json`). Headless handlers are **not** in the manifest (they're runtime-declared by a running service — see §3.5). |
...
| `src/lib/integrations/notifications/store.ts` + its `emitNotification` call sites | Re-point every `emitNotification(ev)` call site to `api.emit({ type: <namespaced>, source, payload })`. The actual call sites are the integration API routes `src/app/api/integrations/[id]/services/[serviceId]/poll/route.ts`, `src/app/api/integrations/[id]/services/[serviceId]/webhook/test/route.ts`, `src/app/api/integrations/webhooks/[integrationId]/[serviceId]/route.ts`, plus `src/lib/integrations/scheduler/jobs.ts` and `src/lib/integrations/services/telegram/notification-handler.ts`. The old store is retired (its data migrated). |
...
```
data/events/
  index.json                     # UNBOUNDED projection index (one entry per event, all time): { eventId -> {type, seq, ts, processing, read, shard, month} }
                                 #   held WARM IN MEMORY; flushed to index.json on checkpoint/interval + shutdown — NEVER rewritten per-emit (ADR-4)
  events/<YYYY-MM>.jsonl         # IMMUTABLE event bodies, one JSON object per line, append-only   (per-emit durable write #1 — O(1) append)
  state/<YYYY-MM>.json           # MUTABLE per-event state for that MONTH: { eventId -> {processing, read, history[], perHandler{} } }
                                 #   bounded to one month (not all-time); the per-emit durable write #2
...
**Boot (at-least-once, FR-005a):** `startEventKernel()` loads `index.json` + the current-month state file into the warm in-memory index/state (fast). Event **bodies are the source of truth**, so if `index.json` is missing or stale relative to the shards (e.g. a crash lost a checkpoint), the kernel repairs it by scanning shards (100k JSON lines is fast, seconds, and happens once at boot, well before any "bell click → list render" budget since the viewer opens after boot). It then reconstructs per-handler FIFO queues from un-acked events and re-dispatches them to active handlers. Handlers whose service isn't running are not active; their events stay `pending` until that handler (re)registers (late catch-up, FR-005b — the `handler_declare` path in §3.5 is the trigger).

**Emission (NFR-001 <100ms) — warm index + bounded hot-path writes:** `emit` = acquire mutex → append the immutable body line to the current-month shard (append-only, O(1) — durable write #1) → create the event's state record → bump the per-type `sequence` → update the **warm in-memory index** + the current-month state (durable write #2: the new state record, bounded to the month) → release mutex → return. Then (non-blocking, after the durable record + return) the dispatch engine fans out; handler completion is not awaited (FR-004).

The **unbounded projection index is NOT rewritten per-emit.** It lives warm in memory and is flushed to `index.json` on a **checkpoint cadence (every N emits / M seconds) and on graceful shutdown** — this is the change that keeps the hot path from doing an O(total-events) full-file rewrite + `fsync` on every emit (the reviewer's M1 point: `index.json` grows with total event count under indefinite retention, so "update the index entry" is a whole-file read-modify-write). Per emit the only durable writes are the two bounded ops above, so **NFR-001 holds at the 100k floor with no dependency change**, and SQLite (ADR-4) remains a genuine last resort, not the only lever.

**Why deferring the index/state flush is crash-safe:** the event **body** (durable per-emit) is the source of truth; the index and mutable state are **derived projections**. If a crash loses in-memory index/state changes made since the last checkpoint, boot repairs the index from the shards and the affected un-acked events are simply **re-dispatched** (FR-005 at-least-once) — idempotent handlers tolerate the redelivery. A lost sub-checkpoint `read` flip at worst reverts one event to unread; no correctness or data-integrity depends on per-emit state/index durability, only body durability does (the single O(1) append).
...
**Namespace validation (FR-023):** a component may register handlers only for event-type namespaces it owns, where "owns" has exactly two paths (implementing FR-023's "match its identifier **or** a granted namespace"):
1. **Owned root from the identifier:** `com.bos.<appId>.*`, where `<appId>` is the component's id — the same id used as `source.appId` (a service's `service.json` `id`, or a built-in app's manifest `id`). No declaration needed.
2. **Granted namespaces:** an explicit, *static* grant list in the component's manifest (built-in `AppManifest.eventNamespaces?: string[]`) or `service.json` (`eventNamespaces?: string[]`), each entry a namespace prefix (e.g. `com.example.shared.*`). This is the path the spec explicitly allows for a component to handle namespaces it does not own by id.

A `register` call is accepted iff the requested `eventType` prefix equals/extends `com.bos.<appId>.` **or** matches a prefix in the component's grant list; otherwise it is rejected with a clear error. The grant list is declared in the static manifest (not per-registration), so a component cannot self-grant a namespace at runtime. Same single-container trust model as ack ownership — a soft integrity guard, documented in the developer docs (FR-026).
...
| **Loopback HTTP** (`http://127.0.0.1:$PORT/api/events/*`) | worker-thread services (emit + ack) | the documented plain loopback-HTTP-to-localhost pattern a worker-thread service uses to reach BOS's own API (`docs/dev/os-shell/virtual-file-system.md`, "Why this matters beyond the Files app"; same loopback shape as the headless-auth `POST /api/secrets/<service>/verify` in `docs/dev/features/headless-client-auth.md`, "Worker-thread services"); works identically for core-adjacent and marketplace services; satisfies "the API must work from service worker-thread context" |
...
Existing mechanism (confirmed in source): `src/lib/integrations/notifications/store.ts` (append-only `notifications.json`, 500-item cap, per-process mutex) + `src/components/desktop/IntegrationsBadge.tsx` (topbar, polls `?count=1`, click = mark-all-read) + `emitNotification(ev)` call sites in the integration API routes (`src/app/api/integrations/[id]/services/[serviceId]/poll/route.ts`, `src/app/api/integrations/[id]/services/[serviceId]/webhook/test/route.ts`, `src/app/api/integrations/webhooks/[integrationId]/[serviceId]/route.ts`), `src/lib/integrations/scheduler/jobs.ts`, and `src/lib/integrations/services/telegram/notification-handler.ts`.
...
**Modify — integration seams:** `src/instrumentation.ts` (start kernel + migration); `src/components/desktop/Topbar.tsx` (swap badge); `src/os/types.ts` (`eventHandlers?` + `eventNamespaces?` on `AppManifest`); `src/lib/apps/store.ts`/`toManifest` (surface installed UI-handler declarations); `src/core/service/types.ts` + `workerIpc.ts` + `ServiceManager.ts` (`event_dispatch` IPC + `dispatchEventToWorker`); re-point `emitNotification` → `api.emit` at its call sites (`src/app/api/integrations/[id]/services/[serviceId]/poll/route.ts`, `src/app/api/integrations/[id]/services/[serviceId]/webhook/test/route.ts`, `src/app/api/integrations/webhooks/[integrationId]/[serviceId]/route.ts`, `src/lib/integrations/scheduler/jobs.ts`, `src/lib/integrations/services/telegram/notification-handler.ts`); retire `src/lib/integrations/notifications/store.ts` + its route.
...
### ADR-4 — File-based storage (shards + per-event state + warm in-memory index), no embedded DB, by default
- **Context:** 100k+ events, indefinite retention, ≤100ms emit (NFR-001), ≤500ms initial list, ≤1s updates (NFR-002/003, SC-002/006). The legacy store is a single 500-capped JSON file — inadequate. The critical sub-decision: how to persist the **unbounded** projection index (one entry per event, all time) without a full-file rewrite per emit.
- **Options:** (a) single JSON file; (b) per-month JSONL shards (immutable bodies) + per-month state files (mutable) + a projection index — with the index either (b1) **rewritten per-emit**, or (b2) held **warm in memory** and flushed on checkpoint/interval + shutdown; (c) embedded DB (SQLite).
- **Decision:** **(b2)** — no new dependency. Rationale: `index.json` is UNBOUNDED (grows with total event count, not per-month), so (b1) "update the index entry per emit" is a full read-modify-write + `fsync` of an ever-growing multi-MB file on every emit, which cannot reliably stay under NFR-001's 100ms at the 100k floor. Holding the index warm in memory and persisting it on checkpoint/interval + graceful shutdown removes that from the hot path; the per-emit durable writes are then only the O(1) append-only shard line + the month-scoped state file (both bounded). Bodies are the source of truth, so the derived index/state are repaired from shards on boot, and lost sub-checkpoint state degrades to the documented at-least-once re-dispatch (FR-005) — no correctness depends on per-emit index durability. The immutable/mutable split still means flipping `read`/`processing` never rewrites a 1MB payload; `writeFileAtomic` for crash safety; per-process mutex for serialization.
- **Consequences:** Fits the "runtime state = files under `data/`" convention and avoids a `package.json`/lockfile change (Constitution VII). Boot rebuild/repair (scanning shards) is one-time and off the viewer-latency path. **The embedded-DB option (c) SQLite remains available as a genuine last-resort escape hatch only if profiling shows the file-based hot path (O(1) shard append + month state write + throttled index checkpoint) still misses the floors at 100k+ — but that is a dependency change requiring explicit approval (VII) and is deliberately not the default, and no longer the *only* lever.**
...
4. **Hot-path write cost at 100k+ (NFR-001 / SC-006).** The unbounded `index.json` is now held warm in memory and flushed on checkpoint/interval + shutdown, so a per-emit is an O(1) shard append + a month-scoped state write — not a full-index rewrite (this was the M1 collision). Residual risks: (a) a *very* heavy single month making that month's state file large — but state is month-bounded by construction, and if a month alone profiles hot, its flush can move onto the same checkpoint cadence (state is a derived projection and tolerates it via at-least-once re-dispatch); (b) a checkpoint flush stalling the emit thread under burst — run checkpoint off the emit critical path or throttle it. Profiling note for `plan`; the embedded-DB escape hatch (ADR-4) remains available but is a last resort, not the default lever.
5. **Namespace-ownership rule (FR-023)** is now fully specified in §3.5: owned root `com.bos.<appId>.*` (from the component id) **plus** a static `eventNamespaces` grant list in the manifest/`service.json` — covering both halves of the spec's "match its identifier OR a granted namespace." Remaining `plan` work is mechanical: wire `eventNamespaces` into `src/os/types.ts` (`AppManifest`) and `service.json` validation, and add the prefix-match check to `api.register`. Still a soft integrity guard under single-container trust — documented in the developer docs (FR-026).
