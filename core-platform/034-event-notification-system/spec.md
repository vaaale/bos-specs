...
- **FR-023**: ~~Handler registration MUST be validated: an app/service can only register handlers for event type namespaces it owns (namespace prefix must match its identifier or a granted namespace).~~ **SUPERSEDED by 037** (`user-specs/core-platform/037-event-namespace-relaxation`, implemented 2026-09-02) — the namespace-ownership gate was removed from `register()`; any component may now register a handler for any event type. The `namespace-not-owned` error is retired. **The `ack` ownership check (FR-022) is preserved** — a component may still only ack handlers it owns. `eventNamespaces` in manifests is advisory-only (documentation, not a gate).
...
- **FR-029**: The implementation MUST include **unit tests** covering: the core event service (emission, dispatch, ack, retry, permanent failure, at-least-once re-dispatch, handler registration/unregistration, ack authorization, payload validation), the read/unread state machine, the handler preference system, and the sequence number assignment. Unit tests MUST clean up after themselves: any events, handler registrations, or preferences created during a test MUST be removed in teardown (afterEach/afterAll) so that tests do not leave residual state in the event store.
- **FR-030**: The implementation MUST include **e2e tests** (Playwright) covering: emitting an event and verifying it appears in the Event Viewer with correct metadata; clicking an event to mark it read and launch a UI handler; the selection dialog flow (multiple UI handlers + "always use"); the configuration page (enable/disable headless handlers, change UI default); the generic event detail view (no UI handler registered); the "Show historical events" toggle; and the bell count accuracy. E2E tests MUST clean up after themselves: any events, handler registrations, preferences, or UI state created during a test MUST be reverted in teardown so that subsequent tests start from a clean slate and the user's event store is not polluted.
