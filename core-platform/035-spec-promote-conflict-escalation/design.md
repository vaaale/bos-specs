...
|---|---|---|---|---|
| 1 | **The reported bug** — feature-promote coupled-repo **pre-check** | `tools/supervisor/lib/coupled-repos.mjs` `coupledConflicts` (called from `promote.mjs` `promote()`) | **each spec store AND user-apps**, via the coupled worktree | `throw new Error(\`promote blocked — ${conflict}\`)` (the exact string in the spec's repro) → create a session + route through `/api/gitfs/reconcile` (loopback); on escalation, report `{ sessionId, devopsConversationId }` instead of throwing. *(Note: this is a `merge-tree` dry-run that fires **before** the code merge — its escalation must make the *branch* mergeable, then the promote proceeds/re-runs; see Risk #8.)* |
...
8. **The coupled pre-check (row 1, the reported bug) is a dry-run, not a live merge.** `coupledConflicts` uses `git merge-tree --write-tree` — no working tree is touched, and it fires *before* the code merge in `promote()`. So its escalation has different completion semantics than the merge-time path (row 2): the session must make the **branch mergeable** (resolve the coupled worktree's branch against base and commit), *not* land the coupled merge itself — the code merge hasn't happened yet. After the session resolves, the promote is re-run and now passes the pre-check and does the normal `promoteCoupled`. This means the `completion` plan for a row-1 session is `kind:"none"` (make-branch-mergeable), whereas a row-2 session is `ff-main`/`commit-merge`/`plumbing-merge` (land it). *Pin which promote re-entry path the Supervisor uses (auto-continue vs. user re-promotes) in `plan.md`.*
