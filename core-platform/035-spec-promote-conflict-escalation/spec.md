...
- **Autonomous-first** (PR-2) is the intended behavior: the agent decides when it can and asks the user only on genuine ambiguity. [NEEDS CLARIFICATION: confirm.]
...
- The **UI form factor** (dedicated window vs desktop panel vs embedded Assistant-conversation view) is decided in the **UI design phase** (a prerequisite to `design`, per the agreed process: UI acceptance before architect). The spec captures the *requirements* (FR-007…FR-011); the visual/interaction design is the mockup.
...
- **Q1 — Target/UI classification**: is the new resolution surface best realized as a dedicated window (a `builtin-app`-shaped surface) vs a desktop panel vs an embedded view within the Assistant app's conversation? (Doesn't change the bos-core spec target, but shapes the UI design and the architect's file plan.) → to be explored in the UI design phase.
- **Q2 — Autonomous-first**: confirm PR-2 (agent decides when it can, asks only on ambiguity) is the desired default vs. always-presenting decisions to the user.
- **Q3 — `awaiting-user` timeout**: should a parked `awaiting-user` session be exempt from the 25-min agent timeout (wait indefinitely until the user returns/abandons), or have its own longer/idle timeout?
- **Q4 — User-apps call site**: confirm the exact Supervisor-process conflict path(s) (architect to pin; recorded as FR-015).
