...
- **FR-015**: Conflicts in the **user-apps** repo MUST route through the pipeline with the **user-apps** working context. The call site is the user-apps entry in `promoteCoupled` (`tools/supervisor/lib/coupled-repos.mjs`), reaching the pipeline via the Supervisor's existing loopback HTTP to `/api/gitfs/reconcile`. (Pinned by the `design` step, D4.)
...
- **D4 — User-apps call site**: **Resolved by `design`.** (Amended by `build-studio/038-user-apps-branch-coupling`: `appPromote` and its module are retired, leaving one path.) The user-apps conflict path is the user-apps entry in `promoteCoupled` (`tools/supervisor/lib/coupled-repos.mjs`), reaching the pipeline via the Supervisor's existing loopback HTTP to `/api/gitfs/reconcile` (FR-015). The Supervisor never imports BOS source and never hard-codes a BOS path.
