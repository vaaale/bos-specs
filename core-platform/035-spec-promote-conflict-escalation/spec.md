...
**Path B — dead-ends (everywhere else).** These conflict paths handle conflicts inline (rebase, merge, or dry-run) and, on conflict, stop with a static error or unhandled throw and **no agent**:

| Call site | Repo | Current behavior on conflict |
|---|---|---|
| **Supervisor feature-promote — coupled-repo pre-check** (`tools/supervisor/lib/coupled-repos.mjs` `coupledConflicts`, called from `promote.mjs` `promote()`) | spec stores + user-apps (via the coupled worktree) | `git merge-tree --write-tree` dry-run; on conflict → `throw new Error(\`promote blocked — ${conflict}\`)` (the exact string in the repro below). **The reported bug.** |
| **Supervisor feature-promote — coupled-repo merge** (`tools/supervisor/lib/coupled-repos.mjs` `promoteCoupled`) | spec stores + user-apps | working-tree path: `merge --abort` + warning "resolve manually in \<root\>"; plumbing path: no conflict handling. Same dead-end class. |
| `src/lib/specs/promote.ts` → `promoteFeature` | user-specs store | `git merge --no-edit <base>`; on conflict → `merge --abort`, returns `{ kind: "conflict", files }`. |
...
```
No agent is triggered; the user must resolve it manually via CLI. (Note: this is the Supervisor's coupled-repo pre-check path, **not** `promoteFeature` in `src/lib/specs/promote.ts` — that is a separate, Build-Studio-driven spec-promote path with a different error shape.)
...
- **FR-012**: `promoteFeature` (`src/lib/specs/promote.ts`) MUST route its base-into-branch conflict through the pipeline with the **user-specs** working context, instead of `merge --abort` + a static `{ kind: "conflict" }`.
- **FR-012a**: The Supervisor's feature-promote coupled-repo paths (`tools/supervisor/lib/coupled-repos.mjs`) MUST route conflicts through the pipeline with the correct working context for each affected store: (1) `coupledConflicts` (the pre-check) — instead of `throw new Error(...)`, it MUST create a session and escalate; (2) `promoteCoupled` (the merge) — instead of `merge --abort` + warning (working-tree) or no handling (plumbing), it MUST route through the pipeline. The working context is per-store: spec stores get the user-specs context, user-apps gets the user-apps context. (The reported bug's site.)
...
- **FR-015**: Conflicts in the **user-apps** repo MUST route through the pipeline with the **user-apps** working context. The call sites are: `appPromote` (`tools/supervisor/lib/app-candidate.mjs`) and the user-apps entry in `promoteCoupled` (`tools/supervisor/lib/coupled-repos.mjs`), both reaching the pipeline via the Supervisor's existing loopback HTTP to `/api/gitfs/reconcile`. (Pinned by the `design` step, D4.)
...
- **Dependency on the 034 event system**: the auto-launch (FR-007) relies on the existing event system's emit + UI-handler dispatch (BS declares a UI handler; a topbar subscriber auto-launches BS on the event). The restart re-emit (FR-024) is an explicit step in the boot sweep — **not** via `redispatchPendingOnBoot` (which only handles pending headless-handler events; this event is UI-only and never `pending`). This feature emits events and declares a UI handler; it does not modify the event system's core dispatch.
...
- **D3 — `awaiting-user` timeout & restart recovery**: a parked `awaiting-user` session waits **indefinitely** — the 25-min timeout applies to the agent's *working* phases only, never to a parked state. On BOS restart (or browser refresh), the system **auto-detects** sessions and recovers them: a session in `working` whose agent run is dead is re-launched by the boot-time sweep; a session in `awaiting-user` is simply restored — the agent does not re-launch until the user answers. The session is persisted (FR-002), so this is restoration of durable state, not re-derivation.
- **D4 — User-apps call site**: **Resolved by `design`.** The user-apps conflict paths are: `appPromote` (`tools/supervisor/lib/app-candidate.mjs`) and the user-apps entry in `promoteCoupled` (`tools/supervisor/lib/coupled-repos.mjs`), both reaching the pipeline via the Supervisor's existing loopback HTTP to `/api/gitfs/reconcile` (FR-015). The Supervisor never imports BOS source and never hard-codes a BOS path.
- **D5 — Pane layout**: the conflict pane **replaces the center artifact-viewer column** while a session is active (it owns the center; the left spec tree stays visible; the pane reverts to the artifact viewer when the session completes or is abandoned). Locked by mockup acceptance.
- **D6 — `awaiting-user` visibility**: **triple-surface amber** — status pill, a full-width banner over the file view, the amber file row/chip in the list, and the amber decision card in the chat. No desktop notification (the user is in the app when a promote is triggered; out of scope). Locked by mockup acceptance.
- **D7 — Decision-timeline depth**: a **compact per-file chip strip** in the pane (✓ resolved · ? awaiting-you · … pending); the full narrative lives in the chat. No in-pane drawer. Locked by mockup acceptance.
- **D8 — Binding UI contract**: `mockup.html` in this spec directory is the **binding UI contract** for the pane's layout, states, and interactions. `design.md` and the implementation MUST conform to it; any deviation requires user re-approval.
