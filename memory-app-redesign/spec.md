...
> Extends `002-memory` (storage substrate) and `021-memory-loops` (automated loops). This spec redesigns the Memory app UI to expose the new episodic buffer, topic sharding, and automated loop management while maintaining backward compatibility with existing USER.md and MEMORY.md functionality.

---

## Problem Statement

After implementing spec **021-memory-loops**, the Memory app only displays the original two-surface model (USER.md + MEMORY.md). Users cannot:
...
The app needs a complete redesign to match the new architecture while preserving existing functionality.
...
- **FR-005**: Two-pane layout showing User Profile (left) and Agent Notes (right) with independent add/edit/delete controls.
- **FR-006**: Budget bars must display current usage vs. limits (1200 chars for USER.md, 2000 for MEMORY.md) with color-coded fill (green <50%, amber 50-80%, red >80%).
- **FR-007**: An info banner must warn that mid-session changes take effect in the next conversation.
- **FR-008**: The Agent Notes pane should display a count of linked topics for entries that index topic files.

### Functional Requirements - Episodes Tab

- **FR-009**: Two-pane layout with episode list (left) and details view (right).
- **FR-010**: Episode list must show: filename (date-conversationId), status badge (Pending/Consolidated), timestamp, turn count, skills used count.
- **FR-011**: Filter controls must allow toggling between All/Pending/Consolidated views.
- **FR-012**: Details view must display all episode metadata (conversationId, createdAt, updatedAt, watermark, skillsUsed, status, skillCandidates) and all body sections from spec 021 FR-001.
- **FR-013**: Action buttons must include "Review Now" (triggers fast loop), "Archive" (moves consolidated episodes >14 days to .Archive/), and "Delete" (with confirmation).
- **FR-014**: Navigation buttons must allow moving between adjacent episodes (Previous/Next).

### Functional Requirements - Topics Tab

- **FR-015**: Two-pane layout with topic list (left) and details view (right).
- **FR-016**: Topic list must show: topic name, entry count, character usage / budget (default 4000), visual budget bar.
- **FR-017**: Details view must display numbered entries with content, timestamps, and consolidation source (episode filename).
- **FR-018**: Actions must include "Add Entry" (with budget validation), "Delete Entry", and "Delete Topic" (with confirmation).
- **FR-019**: A search box must allow filtering topics by name.

### Functional Requirements - Memory Loops Tab

- **FR-020**: Configuration sections for Fast Loop, Slow Loop, and Advanced Settings with all fields from the `memoryLoops` config namespace (spec 021 FR-019).
- **FR-021**: Toggle switches must enable/disable each loop; numeric inputs for intervals/thresholds/batch sizes; dropdowns for model overrides.
- **FR-022**: A run history section must display last execution summaries for both loops, including: timestamp, processed counts, created/updated items, archived count, refusals.
- **FR-023**: Manual trigger buttons must invoke the respective loop immediately with visual feedback (loading state, success/error messages).
- **FR-024**: Save/Reset buttons must persist configuration to Settings API and restore defaults.

### Functional Requirements - Search Tab

- **FR-025**: A search box with query input and search button; optional debounce for performance.
- **FR-026**: Filter buttons to restrict results by source (All/Topics/Episodes/Memory).
- **FR-027**: Results must display: source path with location anchor, content snippet with matched terms highlighted, relevance score, timestamp/status.
- **FR-028**: Clicking a result must navigate to the source in the appropriate tab (implementation may be deferred to Phase 2).

### Functional Requirements - Backend Integration

- **FR-029**: The app must consume these API endpoints:
...
- **FR-030**: If backend endpoints are missing, the app must display appropriate empty states or disabled controls with helpful messages.

### Functional Requirements - State Management

- **FR-031**: The app must use `useOSStore` selectors for OS state (window title, launch/close actions).
- **FR-032**: Dynamic data must be fetched in `useEffect` after mount to avoid client-only initial state.
- **FR-033**: Loading states must be shown during API calls; error states must display user-friendly messages.
...
**Goal**: Create the app skeleton with tab navigation.

**Tasks**:
1. Create `src/apps/memory-app/manifest.ts` (new app ID: `memory-app` or update existing `memory` app)
2. Create `src/apps/memory-app/index.tsx` with 5-tab structure
3. Implement tab switching logic with state management
4. Add lucide-react icons for each tab
5. Style tabs per BOS conventions

**Acceptance**: Tabs render correctly; switching works; active state visible.
...
6. Documentation updates (`docs/usage/apps/memory.md`)
...
- **App ID**: Consider updating the existing `memory` app or creating a new `memory-app` ID. If updating, ensure graceful migration for existing users.
...
- **Documentation**: Update `docs/usage/apps/memory.md` to reflect new tabs and features.
...
```
user-specs/memory-app-redesign/
