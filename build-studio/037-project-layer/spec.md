...
3. ~~**Given** an item-owned store, **When** it is written to, **Then** this rule does not apply — it keeps its existing, separate commit-on-save behavior (see Edge Cases).~~ **SUPERSEDED by `038-user-apps-branch-coupling`:** the rule DOES apply to item-owned stores. They are written through the same feature-branch requirement, via the `020` coupling of `user-apps`.
...
- Item-owned spec stores (a marketplace item's own `spec/` folder) are excluded from the **Project layer** and the migration: they have no Projects, and the "write must target a file inside a Project" rule never applies to them.
- ~~They are also excluded from the entire git workflow (User Stories 3-8).~~ **SUPERSEDED by `038-user-apps-branch-coupling`.** The reasoning here was that an item store is a bare subdirectory of a repo shared with sibling items, and plain git has no directory-scoped branch mechanism, so branching one item would necessarily branch every sibling. The git premise is correct; the conclusion was not, because `data/user-apps` is ALREADY a branch-coupled repo (`020`) mounted whole on the feature branch — branching every sibling is precisely what that mechanism does, and what a feature branch means. Item stores now obey the same branch requirement as every other writable store; only the mount path differs (`user-apps` is a different repo from the `<codeWorktree>/specs/<storeId>` mounts).
...
- **FR-001**: A directory-scanned store's top level MUST hold **Projects** — sub-folders each carrying a `project.json` manifest (`{ label, description? }`) — instead of features directly. A Project's manifest is hidden from directory listings, the same way a store's own manifest is. This does not apply to item-owned stores, which have no Projects (still true after `038-user-apps-branch-coupling`, which changes only their BRANCH rule, not their Project status).
...
- **FR-008**: Once ANY Project has been given the ability to be activated, a write, rename, or delete against a file in a Project with no active session MUST be refused with an error identifying the Project and stating that it must be activated first — it MUST NOT silently fall back to editing the store's base checkout. ~~This requirement does not apply to item-owned stores.~~ **AMENDED by `038-user-apps-branch-coupling`:** it applies to item-owned stores too — refusal is on the missing feature branch rather than a Project, since item stores have no Projects.
